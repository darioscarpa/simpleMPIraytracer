...
void World::getColourForRay(const Ray& ray, Colour& colour_out, int reflections)
{
	if (reflections > World::MAX_REFLECTIONS) return;
	...
	if(closest_object != NULL)
	{
		...
		closest_object->getColourForRay(intersect_point, ray.unitdir, *this, colour_out, ++reflections);
...
void World::calcLightingForSurfPoint(const Object& ob, const Vec3& unit_cam_ray, const Vec3& ob_intersect_pos,
		const Vec3& ob_normal, Colour& colour_out, int reflections)
{
	if (reflections > World::MAX_REFLECTIONS) return;
	...
	if(ob.getMaterial().reflect_fraction != 0.0f)
	{
		...
		getColourForRay(reflected_ray, reflected_colour, ++reflections);
